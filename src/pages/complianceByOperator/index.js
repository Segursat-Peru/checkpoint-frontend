...
import { getSearchEventsWithGroupingOperators } from "../../service/history";
import exportFromJSON from "export-from-json";
import ComplianceTable from "../../components/complianceByOperator/ComplianceByOperatorTable";

const ComplianceByOperator = () => {
	...
	const filteredEvents = () => {
		...
		const filtered = searchEvents.filter(eve =>
			eve.checkpoint.toLowerCase().includes(search)
		);
	...
	const nextPage = () => {
		if (
			searchEvents.filter(eve => eve.checkpoint.toLowerCase().includes(search))
				.length >
			currentPage + 10
		) {
	...
	const onSubmitForm = async data => {
		...
		await getSearchEventsWithGroupingOperators(
			initial_date_value,
			final_date_value
		).then(events => {
			...
			setNameFile(
				`Reporte de cumplimiento por Operador desde ${initial_date_value} hasta ${final_date_value}`
			);
		});
	};
	...
	return (
		<section className="h-screen dark:bg-gray-800">
			...
			<Header />
			<Breadcrumb
				title="Reporte de Cumplimiento por Operador"
...
export default ComplianceByOperator;
